...
const char *input = "121 59 141 21 120 67 58 49 22 46 56 112 53 111 104 130\n"
...
{
	size_t lines = 0;	
	size_t line_size[MAX_LINES] = {0};
	int data[MAX_LINES][MAX_LINE_SIZE];
	...
	for (int i = 0; i < len; ++i) {
		...
		if (c >= '0' && c <= '9') {
			...
			size_t current_line_size = line_size[lines];
			data[lines][current_line_size] = current;
			line_size[lines] += 1;
			current = 0;
		}
